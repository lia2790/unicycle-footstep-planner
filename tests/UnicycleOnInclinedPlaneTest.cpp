...
typedef struct {
    ...
    double inclPlaneAngle = iDynTree::deg2rad(0);
    bool swingLeft = true;
...
bool printOnFileSteps(std::deque<Step> leftSteps, std::deque<Step> rightSteps){
    ...
    posLeft.open("posLeft.txt");
    posRight.open("posRight.txt");

    for (auto step : leftSteps){
        posLeft << step.position.toString() << iDynTree::rad2deg(step.angle) << ' ' << step.impactTime << std::endl;
    }


    for (auto step : rightSteps){
        posRight << step.position.toString() << iDynTree::rad2deg(step.angle) << ' ' << step.impactTime << std::endl;
    }
...
bool printOnFileDesiredTrajectory(UnicyclePlanner& planner,double initTime, double endTime, double dT)
{
    ...
    desiredTrajectory.open("desiredTrajectory.txt");

    double t = initTime;
    iDynTree::Vector2 yDes, yDotDes;

    while (t <= endTime){
        if(!planner.getDesiredTrajectoryPoint(t,yDes,yDotDes))
            return false;
        desiredTrajectory << t << ' ' << yDes(0)<< ' ' << yDes(1) << ' ' << yDotDes(0) << ' ' << yDotDes(1) << std::endl;
        t += dT;
    }    

    desiredTrajectory.close();
...
bool populateDesiredTrajectory(UnicyclePlanner& planner, double initTime, double endTime, double dT, double inclPlaneAngle){
...
bool plannerTest(){
    ...
    clock_t start = clock();
    iDynTree::assertTrue(populateDesiredTrajectory(planner, conf.initTime, conf.endTime, conf.dT, conf.inclPlaneAngle));
